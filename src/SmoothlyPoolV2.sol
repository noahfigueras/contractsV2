...
interface ISmoothlyPoolV2 {
  event Registered(uint64 indexed validatorIndex);
  error RootNotFound();
  error InvalidProof();
  error InvalidIndex();
...
contract SmoothlyPoolV2 is ISmoothlyPoolV2 {
  ...
  function register(
    ...
    // Now register here. Make sure to record timestamp of registration
    // to account for pro-rata-registration also effectiveBalance will be important
    // once MAX_EB is shipped
    registrants[validatorIndex].timestamp = uint64(block.timestamp); 
    registrants[validatorIndex].data = validator; 
    emit Registered(validatorIndex);
  }
