...
contract BeaconOracle is IBeaconOracle {
  ...
  function verifyFeeRecipient(
    ...
    if(root != blockRoot) revert InvalidProof();
    // TODO 
    // To make this persistent in time (not reliable on blockRoot HISTORY_BUFFER_LENGTH)
    // We should concatenate a proof of the state that includes the computed root 
    // generated by the proof. This way we can verify that the slash happened.
